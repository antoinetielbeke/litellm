...
class Router:
    ...
    async def _ageneric_api_call_with_fallbacks_helper(self, model: str, original_generic_function: Callable, **kwargs):
        ...
        try:
            ...
            compacted_response_kwargs: Final = await compact_to_fit(
                self,
                deployment,
                response_kwargs,
                "messages" if getattr(original_generic_function, "__name__", "") == "anthropic_messages" else None,
            )
            async with self._deployment_slot(deployment=deployment, kwargs=kwargs, parent_otel_span=parent_otel_span):
                response = await original_generic_function(**compacted_response_kwargs)
    ...
    async def async_function_with_fallbacks(self, *args, **kwargs):
        ...
        if (
            kwargs.get("original_function") == self._acompletion
            or getattr(kwargs.get("original_generic_function"), "__name__", "") == "anthropic_messages"
        ):
